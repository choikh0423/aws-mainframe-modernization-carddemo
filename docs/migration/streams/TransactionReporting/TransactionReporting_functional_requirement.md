...
| FR-1404 | The report can be launched in process by S-07's CR00 replacement through a launch port that takes the same three values and returns the run's status and the path of the report it produced. | CORPT00C.cbl:462-521 (`SUBMIT-JOB-TO-INTRDR`, `WRITEQ TD QUEUE('JOBS')`) | `TranReportJobTest.launchesTheJobThroughTheReportLaunchPort` |
